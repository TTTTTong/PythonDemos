...
def query_ticker(exchange_id, symbol):
    ...
    sql = "SELECT `time` FROM `market_ticker` WHERE `exchange_id` = %s AND `symbol` = %s"
...
def ticker_handle():
    while True:
        ...
        for item in item_list:
            ...
            db_timestamp_trade = query_ticker(item[0], item[1])
            ...
            if cur_timestamp_2 - db_timestamp_trade > 120:
                ...
                logger.info('begin sleep 120s after restart %s_ticker' % item[2])
                time.sleep(120)
                is_restart = True
...
if __name__ == '__main__':
    ...
    cur0.close()
    # 目前所有交易对
    # ((1, 'btcusdt', 'huobipro'), (2, 'tBTCUSD', 'bitfinex'),
    # (3, 'BTCUSDT', 'binance'), (3, 'ETHBTC', 'binance'),
    # )
